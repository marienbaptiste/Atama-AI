...
claude -p \
  ...
  --append-system-prompt "$(cat .cache/tutor_prompt_rendered.txt)" \
...
- `--append-system-prompt-file` is referenced in the `--bare` help text; if present, prefer it over `--append-system-prompt "$(cat …)"` so the rendered profile does not appear in `ps` argv.
...
2. **Long-lived token.** `claude setup-token` ("Set up a long-lived authentication token (requires Claude subscription)") prints a token; put it in `.env` as `CLAUDE_CODE_OAUTH_TOKEN`. Interactive — the user runs it, never `make`. Use this when the backend runs somewhere the interactive login is awkward.
...
## 6. TUTOR SYSTEM PROMPT (template — render with profile, keep as a versioned file `prompts/tutor.md`)
...
- Idle life: auto-blink (random 2–6 s), subtle procedural sway, `lookAt` camera. **Listening reactions:** while `state=listening` and the server reports speech (VAD `speech_start`), the avatar shifts to an attentive pose and gives a small nod on each detected pause ≥ 300 ms — the あいづち a human tutor would give. While `thinking`, a subtle "considering" idle (gaze up-and-away, slight head tilt).
...
- **Self-barge-in (the avatar interrupting itself).** The mic hears the speakers. Three layers, all required: (1) `getUserMedia` with `echoCancellation: true`, `noiseSuppression: true`, `autoGainControl: false`; (2) while `speaking`, both the client energy check and the server VAD use a **higher threshold** (config `BARGEIN_THRESHOLD_FACTOR`, default 2.0× the listening threshold) and require ≥ 250 ms of sustained speech before firing; (3) the server ignores VAD `speech_start` events during the first 150 ms of playback (loudspeaker onset). M3 acceptance includes a **10-turn conversation on laptop speakers with zero self-interruptions**, in addition to the headphones run.
...
- `.env.example` documents the environment-override names for the automation case only, and stays the **complete inventory** of keys: every key `config.py` reads appears there with its default and a one-line comment, and the settings page is generated from the same schema. A test asserts the three sets — `config.py`, `.env.example`, settings-page schema — match.
- `.gitignore` is part of the spec, not an afterthought. It ignores `.env*` (except `.env.example`), the generated `mcp.json`, `logs/`, `.cache/`, rendered prompts, `*.glb`, model weights, and `backend/tests/fixtures/private/`. `make doctor` verifies with `git check-ignore` that each of those paths is ignored, and fails otherwise.
...
- `mcp.json` generated from `mcp.json.template` + `.env` at startup into `.cache/` (never commit credentials inside it). Credentials reach an MCP server only through that entry's `env` block — never through the claude process environment (§4 allowlist).
- Everything tunable lives in `config.py` reading env with sane defaults; no magic numbers scattered in code. Runtime changes from the settings drawer (§8) update the live config, not `.env`.
...
- **Redaction test.** A test writes a session with fake tokens set in the environment, then asserts that no value from `.env` (and no `Bearer …` header) appears anywhere under `logs/` or `.cache/` except `.cache/mcp.json`. Tool-call args and error strings are the usual leak paths; sanitise at the logging boundary, not at each call site.
- `make check-secrets` — greps the tracked tree for token-shaped strings and for every value currently in `.env` / `settings.json`; wired into `make test`.
...
**M0 — Skeleton & environment doctor.** Repo layout below; **`backend/tools/readonly_gate.py` and the `make check-readonly` / `make hooks` targets first — the Golden Rule gate (§0) exists before any SRS code does, and `make test` cannot pass without it**; `.gitignore`, `.env.example`, `config.py` with the defaults → `settings.json` → env resolution and the settings schema (§11), `backend/constants.py` with the verified CLI findings (§4); `backend/srs/http.py` read-only client (§5); `make doctor` checks: claude CLI present, version vs pinned, `ANTHROPIC_API_KEY` absent from the shell, a trivial `claude -p "respond with OK"` in stream-json with `init.apiKeySource == "none"` and result `OK`; CUDA visible; VOICEVOX reachable on loopback and not on other interfaces; tokens present; every ignored path actually ignored (`git check-ignore`). Prints the §5b status table. Clear actionable error messages.
...
├─ mcp.json.template  .env.example (optional env overrides + key inventory)  .gitignore  Makefile
...
- **DON'T** require a `.env`. The settings page is the configuration interface (§11); env vars are an optional override for automation. **DON'T** send a stored secret back to the browser — `{set, hint}` only.
