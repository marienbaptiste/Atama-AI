...
**This application reads from WaniKani and Bunpro. It never sets, writes, creates, updates, submits, starts, or deletes anything through either API key. Not from the orchestrator, not from a fetcher, not from any MCP server, not from Claude, not from a test, not from a script, not "temporarily".**
...
5. `backend/srs/bunpro_mcp.py`, should an SRS MCP server ever be reintroduced (none exists since ADR-039), registers only the three read tools, and each tool's implementation calls only `get`.
...
It renders and plays, and sends `control`, `settings`, `explain` and `mic_status` messages. **The
page captures the microphone** (ADR-040, user decision 2026-09-15: audio in and out both live where
the student is, so the page can move to another machine later): `getUserMedia` → AudioWorklet →
16 kHz mono PCM16 frames, sent as **binary WebSocket frames** — the one non-JSON message — and
reframed on the server into the same VoiceLoop. The page holds the turn open with push-to-talk
`control: start` / `stop` (`cancel` drops the hold). The sounddevice capture of §9 remains for
a lesson without a page (`--listen` from the terminal) and for `make doctor`. Memory is spec §6b,
search §5c, the launcher §15.
...
### Data flow per turn
1. The page captures the mic (`getUserMedia` with echo cancellation, AudioWorklet, 16 kHz mono PCM16 in 512-sample frames — `frontend/src/capture.ts`, ADR-040) and streams it as binary WebSocket frames; the orchestrator feeds them to the VoiceLoop (`feed_pcm16`). The page holds the turn open with push-to-talk `control: start` / `stop` (`TURN_MODE=ptt`, the default). Without a page, the orchestrator captures the mic itself (`backend/audio.py`).
...
2. **Client construction.** The SRS HTTP client (`backend/srs/http.py`, shared by both fetchers) exposes **only** a `get()` method. There is no `post`/`put`/`patch`/`delete` to call by accident, and a test asserts a non-GET method is not reachable through it.
3. **Tool surface.** No SRS tool is exposed to the model (ADR-039, 2026-09-14): WaniKani and Bunpro reach the tutor only through the Student Profile. Reintroducing one takes a new ADR, read tools only, never a community server with write tools; `--allowedTools` with an explicit read-only tool list stays the belt-and-braces, and gate rule 5 inspects `srs/bunpro_mcp.py` should it reappear. A prompt instruction is **not** sufficient.

A test in the standing suite records every outgoing HTTP request during a full mocked session and asserts all are `GET` (`backend/tests/test_srs_readonly_session.py`).
...
- **Bunpro** (unofficial — treat as fragile): a launch fetch (and Refresh) of JLPT progress, due counts, ghosts and the grammar still in play, rendered into the Student Profile exactly like WaniKani. No tool: the Bunpro MCP server that once exposed it was retired by ADR-039. Wrap every Bunpro call in try/except; on any failure log a warning and continue without it. Never let Bunpro breakage block startup.

**No Bunpro MCP server (ADR-039, 2026-09-14, superseding ADR-023's).** Bunpro has **no official API** — it was deprecated in 2024 and the docs removed; reverse-engineering the site's `/api/frontend/*` endpoints is permitted by staff with the warning that they "may change without warning". ADR-023 wrote `backend/srs/bunpro_mcp.py`, three read tools, after the community servers were disqualified (write tools, a password, or hosted-only). Once the fetch policy below made it a reader of the launch snapshot it duplicated the Student Profile, and the tutor never called it (zero calls in 110 logged turns). It was deleted: Bunpro reaches the tutor through the profile only, and any future SRS tool needs a new ADR.

**Fetch policy — never spam the SRS APIs (user directive 2026-09-09, ADR-024).** WaniKani and Bunpro are contacted **only at app launch and on the student's manual Refresh** (`control: resync`). Nothing else ever calls them — not a timer, not a turn, not an MCP tool. Each launch/refresh stores a **snapshot** (`.cache/srs/<service>.json` with `fetched_at`) and the status registry persists to `.cache/srs/status.json`. **Every launch fetches the latest data** (user directive 2026-09-12); `SRS_CACHE_TTL_S` (default 0) is an opt-in guard that re-uses a snapshot younger than that many seconds instead, for rapid restarts during development. A snapshot in which some endpoints failed is stored with `partial: true` and is **not** re-used: the next launch fetches again regardless of age. An HTTP 429 ends the fetch for that service — no retry (ADR-024), every later request in that fetch refuses with the same reason — and the chip reads `stale` "rate limited" (an older snapshot, if any, is served). Nothing but the orchestrator reads the snapshot (a Bunpro MCP server did until ADR-039), so no token enters any other process, and `mcp.json` contains no credential. Measured 2026-09-09: launch fetch of both sources 4.6 s (budget 10 s), 6 Bunpro calls + 5 WaniKani calls; **8 Bunpro calls as of 2026-09-12** (the beginner, adept and seasoned levels joined the ghosts — §8b). `/user_stats/srs_level_overview` is verified and pinned in `constants.BUNPRO_READ_ENDPOINTS` but not fetched at launch.

**Bunpro credential:** the **Account API Token from Bunpro → Settings → API**, sent as `Authorization: Token token=<token>` **together with the query parameter `dangerously_authenticate_using_api_token=true`** and browser-like `Origin`/`Referer` headers of `https://bunpro.jp` — verified live 2026-09-09: without the parameter every endpoint returns 401 `AUTH_USER_DENIED`. (Bunpro's own naming flags this token as a full-account credential; the read-only transport is the mitigation.) `srs_level_details` takes a **named** `level` (`beginner|adept|seasoned|expert|master`); a numeric or missing level returns HTTP 500. That is the whole credential story — no email, no password, no browser-cookie scraping. Base origin `https://api.bunpro.jp` (what the current read-only community server hardcodes; confirm at M1, then it is a constant in `srs/http.py` — never a setting, §0 rule 7). Read endpoints: `/user`, `/user/due`, `/user/queue`, `/user_stats/jlpt_progress_mixed`, `/user_stats/srs_level_details?reviewable_type=Grammar`, `/user_stats/srs_ghost_level_details?reviewable_type=Grammar`, `/user_stats/forecast_daily`. Politeness: ≥ 1 s between requests (2 s × the session-start calls, eight since 2026-09-12, would not fit the 10 s budget — measured 2026-09-09), and **no retry at all** after a 429. Every response is validated against a pinned fixture so an upstream change fails loudly into the `bunpro` status chip instead of silently feeding garbage to the tutor. The token is used by the launch fetch only and never enters the claude subprocess or any MCP server.
...
| `search`       | `disabled` · `ok` (ready marker seen) · `down` (no ready signal, or the engine unreachable) · `used` | the search server's ready marker (§4), then its `tool_use`/`tool_result` pairs on the stream (§5c) |
| `brain`        | `starting` · `ready` · `thinking` · `rate_limited` (from `rate_limit_event`) · `fallback` (fallback model active) · `restarting` · `error` | the ClaudeSession event stream |
...
- `make doctor` prints the same table on the console, so "is search up" has one answer in both places.
...
- **A separate MCP server** — `backend/search_mcp.py`, exposing one read tool, `search` (`query`, `category`, `language` ∈ {`ja`, `en`, `all`} — an allowlist, default `ja`; anything else falls back to the default). Since ADR-039 it is the only MCP server, and it never gains an SRS tool (§0 rule 5, §5).
...
- **Each page has its own outbox** and sends never block the conversation: `mic_level` is coalesced (only the newest is kept), everything else — a sentence, a state, a barge-in, a settings echo — is delivered in order however slow the page; a page whose outbox reaches `OUTBOX_MAX` has stopped reading and is closed so it reconnects. A page that drops off the socket **mid-hold** has its hold cancelled as its own ALT GR would, so the microphone is never wedged open by a vanished tab; if it was the page sending audio, the loop is told the microphone is gone (`missing` until a page sends again). **Reconnect keeps the microphone:** the page's capture runs on regardless of the socket — frames that meet a closed or reconnecting socket are dropped, and on the next open the page re-reports its `mic_status`, so the server's picture is right again within one message.
...
- **Self-barge-in (the avatar interrupting itself).** The mic hears the speakers. With push-to-talk (the default, §9) the tutor's own voice cannot end a turn, which is the strongest layer. For `vad` mode, three layers, all required: (1) capture-side echo cancellation — the page owns the microphone (§2, ADR-040), so `getUserMedia` is asked for `echoCancellation`, `noiseSuppression` and `autoGainControl`, and the browser's AEC is the first layer (in a terminal lesson, where sounddevice captures, this layer is the OS/device's); (2) while `speaking`, the server VAD uses a **higher threshold** (config `BARGEIN_THRESHOLD_FACTOR`, default 2.0 — note the server VAD emits a *probability* capped at 1.0, so the factor divides the headroom to certainty, `1-(1-t)/f`, giving 0.75 at t=0.5: multiplying would put the bar at 1.0 and silently disable barge-in, verified 2026-09-09) and require ≥ 250 ms of sustained speech before firing; (3) the server ignores VAD `speech_start` events during the first 150 ms of playback (loudspeaker onset). M3 acceptance includes a **10-turn conversation on laptop speakers with zero self-interruptions**, in addition to the headphones run.
- **The page captures the audio** (ADR-040, 2026-09-15; the original ADR-006 design, built at last): `frontend/src/capture.ts` opens the microphone once the page has been touched (the same gesture that unlocks playback), runs it through an AudioWorklet (`capture_worklet.ts`, the pure resampler and framer in `pcm.ts`) and streams 16 kHz mono PCM16 frames to the server for the whole lesson — the VAD needs the room between turns for its level meter, its listening reactions and hands-free mode, exactly as the sounddevice thread gave it. The page also controls the turn: push-to-talk `control: start`/`stop`/`cancel`, and **losing window focus (`blur`) cancels a capture in progress** (§9b) — a key release the page never sees must not leave the hold open. The chosen microphone is the page's own (Settings → Sound, remembered per browser — it is a device of the machine the page runs on, not a server setting); its recovery is §9's, done by the page: chosen device gone → the default (`fallback`) and back on `devicechange`; none at all → `missing`, tried again every 2 s and on `devicechange`; the track ends mid-lesson → `lost`, reopened; permission refused → `denied`, tried again on the next press. Every state goes to the server as `mic_status` and to every screen as `service_status: microphone`. **Between the page and the terminal, the sounddevice path of §9 is the terminal's**: a lesson without a page still captures locally.
- **DON'T** add build complexity: no React, no state library. One page, a few modules.
- **The phone (ADR-041, user request 2026-09-16).** The same page, served to a phone on the same
  network from a second listener (§15 "Remote"), in a **mobile mode** that switches on only for a
  coarse pointer on a narrow screen (`frontend/src/mobile.ts` `wantsMobile`; `#m=1` / `#m=0` in the
  URL forces it for testing) — **the desktop page is unchanged** (user, 2026-09-16). In mobile mode
  the status bar, the New topic and Stop buttons and the Activity card move into a **hamburger
  drawer** (top left); the drawer also opens the conversation as a full-screen overlay and the
  settings panel; the dock keeps the talk button (hold to talk, `pointercancel` cancels) and the
  hint; subtitles show while the conversation overlay is closed. The page asks for a screen wake
  lock while a lesson is running there (and on the desktop only when `KEEP_AWAKE` is on).
- **Settings → Remote** (§11): keep the computer awake, serve the phone page, its port, and the QR
  code — the code is shown only on a page open on the computer itself, never on a phone.
...
the session, and neither may need a relaunch. **With a page attached the page does all of this**
(ADR-040, §8: `capture.ts`, the same states, `mic_status` to the server); what follows is the
terminal lesson's path, `backend/audio.py`, which also serves `--speak` playback without a page.
...
- **Host APIs (2026-09-15):** PortAudio publishes one Windows device under four APIs, two of which
  cannot open our fixed rates (WASAPI: no resampling in shared mode; WDM-KS: no blocking API —
  measured 2026-09-09). Those two are a **blocklist**; every other API — MME and DirectSound on
  Windows, ALSA, PulseAudio, JACK and OSS on Linux — is usable and ranked in that order, so the
  same code lists and opens devices on Linux (the reference topology, §15). ALSA's aliases of the
  default (`default`, `pulse`, `pipewire`, `dmix`, `dsnoop`) are hidden from the pickers like the
  Sound Mapper. **Not verified on Linux yet** (ROADMAP live checks): open a stream at 16 kHz in and
  24 kHz out on an ALSA box, then pin the finding.
...
- **The settings interface** is the frontend settings page (the M3 drawer grown up): every key in `config.py`, grouped (Account & tokens · Brain · Voice · Sound · Display · Remote · Advanced), with type-checked inputs, the default shown, and a description. Secrets use masked inputs; the server echoes secrets back **only** as `{set: true, hint: "…abcd"}` — the full value never leaves the backend once stored. "Does it work" is answered by the status chips (§5b) and by `make doctor`, not by per-service Test buttons on the page (none exist; the original design had them). Changes apply live where possible (the audio devices today; voice, VAD, display planned), trigger the documented respawn for `model`, and re-run the session-start fetch for tokens. **First run:** no keys → the first-run card of the next bullet; the lesson runs regardless (SRS is optional, and `claude` is checked by the doctor).
- **Keys added 2026-09-16** (group `remote`, ADR-041): `KEEP_AWAKE` (block system sleep while the tutor runs — Windows `SetThreadExecutionState`, Linux `systemd-inhibit`, macOS `caffeinate`; the display may still sleep, the page holds its own screen wake lock), `REMOTE_ENABLED`, `REMOTE_HOST` (empty = this machine's LAN address; a private-network address, never a wildcard, never public), `REMOTE_PORT` (8443). All apply live in a voice lesson. **There is no stored key:** the phone's session key is generated at every launch, held in memory only, carried by the QR code, and replaced by the panel's New key (user, 2026-09-16). **A phone may not change secrets or the `remote` group** — the server refuses those from a socket that did not come from loopback, and says so.
...
- `mcp.json` is generated directly by `backend/tools/mcp_config.py` from the resolved config at startup into `.cache/` (there is no template file; never commit credentials inside it). Credentials reach an MCP server only through that entry's `env` block — never through the claude process environment (§4 allowlist); today the one MCP server, search, needs none (§5).
...
**M4 — Sensei brain.** Memory and context (§6b, ADR-031/032) — the turn log, the start-of-session read, the end-of-session summariser, and *last* the pre-emptive session rotation, which is the only piece here that can break a working conversation and so ships after several real lessons on the rest. `CONTEXT_ROTATE_AT` unset means never rotate, and that stays a supported configuration. Then: tutor prompt tuning with the real profile, Bunpro MCP (written per ADR-023, then retired by ADR-039 — §5), §5b status indicators wired to real state, `control: resync` from the UI. The fetchers and profile renderer already exist from M1; M4 is about the tutor *using* them well and the user *seeing* that it does. Acceptance: with a real WaniKani token, the tutor demonstrably uses ≥ 3 recent unlocks in a 5-minute conversation (visible in logs); Bunpro absent → clean degradation with the chip reading `disabled`; WaniKani offline with a warm cache → chip reads `stale`, profile still present.
...
**Remote — the phone on the same network (ADR-041, user decision 2026-09-16).** Off by default; on, the orchestrator opens a **second listener** on this machine's own LAN address (never `0.0.0.0`, never loopback, and **only a private-network address** — RFC 1918 or link-local; `REMOTE_HOST` empty = the default route's IPv4), **HTTPS only** with a self-signed certificate generated once into the per-user state directory (`~/.atama-ai/remote/`, SAN = the address, 800 days — inside Apple's limit for a trusted leaf), and admits a socket only when the client is itself on a private address, its `Origin` is that listener's own, and its URL carries **this run's session key** (`?k=…`, compared in constant time). The key is generated at every launch and never written anywhere (user, 2026-09-16); New key in the panel replaces it for the rest of the run. The QR code (Settings → Remote, on a page open on the computer only) encodes `https://<address>:<port>/?k=<key>`; the page stores the key in its own browser for reconnects and drops it from the address bar — after a relaunch the phone scans again. Everything else stays loopback: VOICEVOX, SearXNG, the loopback page. `make doctor`'s LAN check is unchanged (the loopback port must still answer nowhere else); the remote port is the one deliberate exception. **What the phone user has to do once:** accept the certificate warning (Android Chrome) or install and trust the certificate profile (iPhone) — without that the browser refuses the microphone, because `getUserMedia` needs a secure context. A public network is never the intent: the certificate and key make the LAN safe, not the internet.

**Linux (reference):** everything on the host. Browser, backend, Docker, `claude` login — one machine, one user, one home directory. The backend's audio (`--speak` playback and the terminal lesson's capture, §9) goes through PortAudio's ALSA host API, which needs the system `libportaudio2`; the page's capture and playback need nothing but the browser.
...
- **Mic:** captured by the **page** (ADR-040), which runs in the Windows browser — so WSL2 needs no audio device at all; capture and playback are both browser-side. (Before 2026-09-15 the orchestrator captured with sounddevice, which WSL2 could not do by default.) A terminal lesson without a page still needs a device the backend can open, and that remains unverified on WSL2.
