...
- **Test** — The gate is tested against a fixture tree of deliberate violations, one per rule,
  ...
  client; a fourth MCP tool (rule 5, which inspects `srs/bunpro_mcp.py` only if one exists — since
  ADR-039 it guards against that file being reintroduced); a write-scope string outside the doctor; an `https://` literal in
...
- **Test** — Against the V0.5 fixtures: level, per-stage counts, ~30 recent unlocks (kanji +
  ...
  a full fetch is `GET` — the standing version over a whole mocked session (launch fetches and
  `resync`; it called the Bunpro MCP tools until they were retired 2026-09-14, ADR-039) is
  `backend/tests/test_srs_readonly_session.py` (2026-09-12). Pagination follows `pages.next_url` up to `MAX_PAGES` (20) and a capped
...
### 13. Bunpro fetcher — `backend/srs/bunpro.py` — **M1**

Treated as fragile by design (ADR-010). The fetcher lands in **M1** on the GET-only client,
alongside WaniKani, and is handled exactly like it: fetched at launch and on Refresh only
(ADR-024), stored as `.cache/srs/bunpro.json`, rendered into the Student Profile, read by the
study plan and the page's marks in Python. **The Bunpro MCP server (`backend/srs/bunpro_mcp.py`,
M4, per V0.7 / ADR-023) was retired 2026-09-14 (ADR-039)** — in 110 logged turns the tutor never
called it, and the profile already carried its data. Its tests (`test_bunpro_mcp.py`) went with
it; the `bunpro_mcp` status service is gone. No SRS tool of any kind comes back without a new ADR.
...
- **Test** — Every call path wrapped: malformed response, auth failure, timeout, and total
  ...
  from the resolved config into `.cache/` (no template) and names one server, `search`
  (ADR-028), only when `SEARXNG_URL` is set; with search off the tutor is spawned with no MCP
  config at all. A test asserts no credential is ever written to a tracked path or into
  `mcp.json`. **Read-only (ADR-021):** the fetcher is built on the GET-only
  `backend/srs/http.py`; the recording-transport session test asserts every request is `GET`.
- **Validate** — Live JLPT progress + ~15 recent/ghost grammar points appear in the rendered
  profile. Break the credential on purpose → the `bunpro` chip reads `stale` (with a snapshot)
  or `error` (without), conversation continues.
- **Gate M4b** — Bunpro fully removed → startup unaffected, conversation unaffected, chip
  `disabled`. (The former "Bunpro MCP broken → chip `failed`" half was dropped 2026-09-14 with
  the server, ADR-039.)
...
- **Test** — `logs/session-<timestamp>.jsonl` opens with a header record (resolved config with
  ...
  session that triggers a tool error and a WaniKani auth error; assert no configured secret
...
| 13   | ~~Claude ↔ Bunpro MCP~~             | Retired 2026-09-14 with the server (ADR-039); Bunpro reaches the tutor through step 0b | M4 |
...
10. **Read-only guarantee (Golden Rule §0, ADR-021)** — `make check-readonly` runs first and
   ...
   and `resync`) reports zero non-`GET` requests; the violation fixture
